...
// Идеально. ЗЫ Никита, если не трудно, можешь закомментить))
 ...
 var count:integer;
  // Процедура чекает можно ли в данную клетку впихнуть кусок корабля, если
  //рядом стоят корабли то возращает - нет
 begin
 ...
 begin
 while counter<>20 do //счётчик на всякий случй
  begin

 n:=10;
 for I := 1 to n do
   for j := 1 to n do
     P2F[i,j]:='S';   //заполняем матрицу морем

 Randomize;
 exiter:=False;
 Ship_size:=4;
 ship_amount:=1;
  for k := 1 to 4 do      //количество размеров кораблей
  begin
    ...
    repeat
      exiter:=False;
      repeat
        ib:=Random(n)+1;  //рандомим начальную координату))
        jb:=Random(n)+1;  //чекаем что б влез корабль
      until not (quick_math(ib,jb)) or ( (jb+ship_size-1>10) and (ib+ship_size-1>10)) ;
      direction:=random(2)+1;           //рандомим направление
      if ship_size>1 then
      begin
      case direction of
      1:
      if  (ib+ship_size-1<=10) then
        ...
        exc:=True;
        for I := 1 to ship_size-1 do    //чекаем влезет ли
        if Quick_math(ib+i,jb) then
        exc:=False;
         if exc  then
         for I := 1 to ship_size-1 do
         begin
         P2F[ib,jb]:='K';       //если влазит записываем, снизу тоже самое
         P2F[ib+i,jb]:='K';
         exiter:=true;
         end;
